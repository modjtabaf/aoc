...
using RawOrderingRules = std::vector<std::array<int, 2>>;
using OrderingRules = std::unordered_map<int, std::vector<int>>;
using PageUpdate = std::vector<int>;
using PageUpdates = std::vector<PageUpdate>;

RawOrderingRules sample_raw_ordering_rules{
    {47, 53},
    {97, 13},
    {97, 61},
    {97, 47},
...
RawOrderingRules raw_ordering_rules{
    {73, 89},
    {78, 59},
    {78, 71},
    {32, 66},
...
void make_ordering_rules(const RawOrderingRules& raw_rules, OrderingRules& rules)
{
...
bool is_page_order_correct(const PageUpdate& page_update, const OrderingRules& rules)
{
    int prev_page = page_update[0];
    for (auto page: page_update)
    {
        if (page != prev_page)
        {
            auto it = rules.find(page);
            if (it != rules.end() && std::find(it->second.begin(), it->second.end(), prev_page) != it->second.end())
                return false;
        }
        prev_page = page;
    }
...
int part1(const PageUpdates& page_updates, const OrderingRules& rules)
{
    int ret = 0;
    for (const auto& page_update: page_updates)
    {
        if (is_page_order_correct(page_update, rules))
        {
            ret += page_update[(page_update.size() - 1) / 2];
        }
    }
    return ret;
}

void fix_page_order(PageUpdate& page_update, const OrderingRules& rules)
{
    int prev_page = page_update[0];
    int k = 0;
    for (auto page: page_update)
    {
        if (page != prev_page)
        {
            ...
            if (it != rules.end() && std::find(it->second.begin(), it->second.end(), prev_page) != it->second.end())
            {
                page_update[k] = prev_page;
                page_update[k - 1] = page;
                page = prev_page;
            }
        }
        prev_page = page;
...
int part2(PageUpdates& page_updates, const OrderingRules& rules)
{
    int ret = 0;
    for (auto& page_update: page_updates)
    {
        if (is_page_order_correct(page_update, rules))
            continue;
        ...
        do
        {
            fix_page_order(page_update, rules);
        } while (!is_page_order_correct(page_update, rules));
...
int main()
{
    OrderingRules sample_ordering_rules;
    make_ordering_rules(sample_raw_ordering_rules, sample_ordering_rules);

    OrderingRules ordering_rules;
    make_ordering_rules(raw_ordering_rules, ordering_rules);

    std::cout << part1(sample_pages, sample_ordering_rules) << std::endl;
    std::cout << part1(pages, ordering_rules) << std::endl;
    std::cout << part2(sample_pages, sample_ordering_rules) << std::endl;
    std::cout << part2(pages, ordering_rules) << std::endl;
    return 0;
}
