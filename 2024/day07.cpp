...
using Equation = std::vector<int64_t>;
...
{
    int64_t n = std::floor(std::log10(b)) + 1;
    return int64_t(a * std::pow(10, n) + b);
}

bool recursion(int64_t left, Equation::const_iterator begin, Equation::const_iterator end, int64_t desired_result, bool use_concatenation)
{
    return (begin == end) ? (left == desired_result) : (
        recursion(left * *begin, begin + 1, end, desired_result, use_concatenation) ||
        recursion(left + *begin, begin + 1, end, desired_result, use_concatenation) ||
        (use_concatenation && recursion(concat(left, *begin), begin + 1, end, desired_result, use_concatenation)));
}

bool is_equation_valid(const Equation& equation, bool use_concatenation)
{
    return recursion(equation[1], equation.begin() + 2, equation.end(), equation[0], use_concatenation);
}

int64_t solve(const Equations& equations, bool use_concatenation)
{
    int64_t ret = 0;
    for (const auto& equation: equations)
        if (is_equation_valid(equation, use_concatenation)) ret += equation[0];
    return ret;
}

int main()
{
    std::cout << solve(sample_equations, false) << std::endl;
    std::cout << solve(equations, false) << std::endl;
    std::cout << solve(sample_equations, true) << std::endl;
    std::cout << solve(equations, true) << std::endl;
    return 0;
}
