...
bool eval_pattern(const std::string_view& pattern, const Towels& towels)
{
    ...
    {
        if (towel.empty()) continue;
        // std::string_view str{p, len};
        found = pattern.starts_with(towel) && eval_pattern({p + towel.size(), len - towel.size()}, towels);
        if (found)
        {
            // std::cout << "matched " << str << " with " << towel << "\n";
            p += towel.size();
            len -= towel.size();
            break;
        }
...
int part1(Patterns& patterns, const Towels& towels)
{
    int ret{0};
    for (auto& pattern: patterns)
    {
        auto foo = pattern;
        // pattern.clear();
        if (eval_pattern(pattern, towels)) ret++;
        // pattern = foo;
    }
    return ret;
}
...
int main()
{
    ...
    // for (auto& s: towels) std::cout << s << "\n";
    std::cout << part1(sample_patterns, sample_towels) << std::endl;
    // std::cout << part1(patterns, towels) << std::endl;
    // std::cout << part1(towels, towels) << std::endl;
    return 0;
}
