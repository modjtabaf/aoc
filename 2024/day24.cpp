...
uint64_t part1(const Values& init_values, Gates& gates, Values& wires)
{
    ...
    do
    {
        ...
        for (auto& gate: gates)
        {
            ...
            gate.evaluated = true;
            wires[gate.out] = v;
            // std::cout << __LINE__ << ":" << gate.out << std::endl;
            n_unknowns++;
        }
        // std::cout << __LINE__ << ":" << n_unknowns << std::endl;
    } while (n_unknowns > 0);
...
void part2(Gates& gates, int depth)
{
    ...
    // for(int n=0;; n++)
    // {
    //     std::sprintf(label, "x%02d", n);
    //     if (!init_values.contains(label)) break;
    //     if (init_values.at(label)) x |= bit;
    //     // std::cout << label << ":" << init_values.at(label) << ", x = " << x << "\n";
    //     bit <<= 1;
    // }
    ...
    // for(int n=0;; n++)
    // {
    //     std::sprintf(label, "y%02d", n);
    //     if (!init_values.contains(label)) break;
    //     if (init_values.at(label)) y |= bit;
    //     bit <<= 1;
    // }
